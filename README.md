...
Daily automation: watch [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) upstream, and build an **unsigned** Windows x64 desktop installer whenever upstream publishes a new release tag (`dsh-v*`).
...
| 环节 | 说明 |
|---|---|
| 触发 | 每天 **UTC 18:00（北京 02:00）** 自动；也可以在 Actions 页手动 `Run workflow` |
| 打包来源 | 默认 = 上游**最新发布的 tag**（`dsh-v*`）。手动跑时可以把 `source` 选成 `master`（跟最新提交）；想让定时任务也跟 master，就在 Settings → Secrets and variables → Actions → **Variables** 里建 `DEFAULT_SOURCE=master` |
...
**要跟 master 最新提交**：见上表 `DEFAULT_SOURCE`（定时任务）或手动跑时选 `source=master`。
